...
contract Elections {
    ...
    event ElectionCreated(uint256 id, string name, uint256 endDate, address creator, string _creatorName);
    ...
    function createElection(string memory _name, string memory _creatorName, uint256 _endDate) public {
        require(_endDate > block.timestamp, "End date should be greater than start date");
        ...
        emit ElectionCreated(electionCount, _name, _endDate, msg.sender, _creatorName);
    }

    function getElections() public view returns (string[] memory, uint256[] memory, uint256[] memory, bool[] memory) {
    ...
    function addCandidate(uint256 _electionId, uint256 _candidateId, string memory _candidateName) public {
        Election storage election = elections[_electionId];
        require(election.creator == msg.sender, "Only the creator of the election can add candidates");
        require(election.isActive, "Election is not active");
        require(bytes(election.candidates[_candidateId]).length == 0, "Candidate already exists");
    ...
    function castVote(uint256 _electionId, uint256 _candidateId) public {
        Election storage election = elections[_electionId];
        require(election.isActive, "Election is not active");
        require(!election.voters[msg.sender], "Voter has already voted for this candidate");
    ...
    function endElection(uint256 _electionId) public {
        Election storage election = elections[_electionId];
        require(election.creator == msg.sender, "Only the creator of the election can end it");
    ...
    function getElectionData(uint256 _electionId) public view returns (string memory, uint256, uint256, bool, string[] memory) {
    Election storage election = elections[_electionId];
    ...
    for (uint256 i = 1; i <= election.candidateCount; i++) {
            candidatesNames[index] = election.candidates[i];
            index++;
        }
    
    return (election.name, election.startDate, election.endDate, election.isActive, candidatesNames);
}

function getCandidateVotes(uint256 _electionId, uint256 _candidateId) public view returns (uint256) {
    Election storage election = elections[_electionId];
    require(bytes(election.candidates[_candidateId]).length != 0, "Candidate does not exist");
